...
int main(int argc, char *argv[]) {
	...
	scanf("%f", &edad);
	printf("Ingrese la antiguedad de la persona en la empresa: ");
	scanf("%f", &antiguedad);
	cout << endl;
	
	if (edad >= 60 && antiguedad < 25){
		printf("La persona queda adscrita a 'Jubilación por edad'");
	} else if (edad < 60 && antiguedad >= 25){
		printf("La persona queda adscrita a 'Jubilación por antiguedad joven'");
	} else if (edad >= 60 && antiguedad >= 25){
		printf("La persona queda adscrita a 'Jubilación por antiguedad Adulta'");
	} else {
		printf("La persona no cumple con lo requisitos para quedar adscrita");
	}
